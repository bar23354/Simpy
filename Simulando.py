    ...
    def run(self):
        ...
        with self.ram.get(self.instrucciones) as req:
            ...
            with self.procesador.request() as req_cpu:
                ...
                # Ensure instrucciones is greater than 0 before releasing memory
...
for num_proceso in num_procesos:
    ...
    env.run()
    tiempos_promedio.append(sum(TiempoConcluido) / len(TiempoConcluido))

plt.plot(num_procesos, tiempos_promedio, marker='o')
plt.title('Gráfica de Tiempo Promedio vs. Número de Procesos')
plt.xlabel('Número de Procesos')
plt.ylabel('Tiempo Promedio de Finalización')
plt.show()